...
class User(BaseModel, db.Model):
    ...
    @property  #把方法变属性 可以直接调用 user.password_hash=password_hash print(user.password_hash)
    def password_hash(self):
        raise AttributeError(u'不能访问该属性')

    @password_hash.setter  #setter 设置方法
...
class Order(BaseModel, db.Model):
    ...
    status = db.Column(  # 订单的状态
        ...
        default="WAIT_ACCEPT", index=True)  #这个字段加索引 加快速度
