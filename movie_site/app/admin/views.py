...
from werkzeug.utils import secure_filename #保证上传文件路径安全
import uuid
import datetime
...
def tag_list(page=None):
    ...
    page_data = Tag.query.order_by(
        Tag.addtime.desc()
    ).paginate(page=page, per_page=10)  # 按时间顺序，分页器一页几个per_page， page第几页
...
def movie_add():
    ...
    if form.validate_on_submit():
        ...
        if not os.path.exists(app.config["UP_DIR"]):
            ...
            os.chmod(app.config["UP_DIR"], "rw")#授权可读可写
...
def role_edit(id=None):
    ...
    if request.method == "GET":
        auths = role.auths
        # get时进行赋值。应对无法模板中赋初值
        form.auths.data = list(map(lambda v: int(v), auths.split(",")))#逗号分隔还原回去
    if form.validate_on_submit():
        data = form.data
        role.name = data["name"]
